...
class GopeedClient:
    ...
    async def stop_task(self, task_id: str) -> None:
        with contextlib_suppress():
            await self.client.delete(f"{self.base_url}/api/v1/tasks/{task_id}", headers=self.headers())
...
class DownloadManager:
    ...
    async def process(self, task: DownloadTask) -> None:
        try:
            ...
            await self.poll_gopeed(task, target)
    ...
    async def poll_gopeed(self, task: DownloadTask, target: Path) -> None:
